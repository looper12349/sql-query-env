"""
Test script for EASY task: simple_lookup
Runs a full episode against the live HF Space or local server.

Usage:
    uv run python scripts/test_easy.py
    uv run python scripts/test_easy.py --local   # test against localhost:8000
"""
...
# Scripted steps: (action_type, query, description)
STEPS = [
    (
        "execute",
        "SELECT name, salary FROM employees WHERE dept_id = 1",
        "WRONG column name — agent hallucinated 'dept_id' instead of 'department_id'",
    ),
    (
        "execute",
        "SELECT * FROM employees WHERE department_id = 1",
        "SELECT * penalty — lazy query, no column aliases, no JOIN",
    ),
    (
        "execute",
        ...
        "CORRECT query — proper JOIN, aliases, ORDER BY",
    ),
    (
        "submit",
        "SELECT e.name AS employee_name, e.salary "
